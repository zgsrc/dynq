...
module.exports = function(schema, description) {
    ...
    if (description.Table.GlobalSecondaryIndexes) {
        description.Table.GlobalSecondaryIndexes.forEach(function(index) {
            me[index.IndexName.camelize(false)] = (hash, range, op) => {
                var query = me.query().index(index.IndexName);
                if (hash) return query.match(hash, range, op);
                else return query;
            };
        });
    }
...
function Query(connection, table, key, op) {
    ...
    this.match = function(hash, range, operator) {
        options.KeyConditions = { };
        
        options.KeyConditions[key[0]] = {
            AttributeValueList: [ encode(hash) ],
            ComparisonOperator: "EQ"
        };
        
        if (range && key.length > 1) {
            var op = "EQ";
            if (operator) {
                op = operators[operator.spacify().toLowerCase()];
            }
            
            options.KeyConditions[key[1]] = {
                AttributeValueList: [ encode(range) ],
