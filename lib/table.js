...
module.exports = function(schema, description) {
    ...
    this.save = function(obj, cb) {
        me.edit(obj).change(obj).update(cb);
    };
    ...
    this.getPart = function(obj, attributes, cb) { 
        me.schema.connection.getPart(name, extractKey(obj), attributes, cb); 
    };
    
    this.getAll = function(objs, attributes, cb) {
        me.schema.connection.getAll(name, objs, attributes, cb);
    }
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////
    this.query = function() { 
        return new Query(me.schema.connection, name, key, "query"); 
    };
    
    this.scan = function() { 
        return new Query(me.schema.connection, name, key, "scan"); 
    };
    
    this.write = function(obj) { 
        return new Write(me.schema.connection, name, key, obj); 
    };
        ...
        connection[op](options, function(err, data) {
            if (err) cb(err);
                ...
                if (selectItems) {
                    ...
                    connection.deleteAll(table, items.map(extractKey), null, function(err) {
                        if (err) cb(err);
                        else cb(null, { last: data.LastEvaluatedKey, count: data.Count });
                    });
                }
                else {
        ...
        async.doWhilst(function(cb) {
            ...
            if (err) cb(err);
                ...
                if (selectItems) {
                    connection.getAll(table, items.map(extractKey), function(err, items) {
                        if (err) cb(err);
                        else cb(null, { items: items, count: count });
                    });
                }
                else if (deleteItems) {
                    connection.deleteAll(table, items.map(extractKey), null, function(err) {
                        if (err) cb(err);
                        else cb(null, { count: count });
                    });
                }
                else {
