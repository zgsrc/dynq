...
module.exports = function(connection) {
    ...
    this.createTable = function(definition, cb) {
        ...
        options.AttributeDefinitions = Object.keys(columns).map(function(name) {
            return { AttributeName: name, AttributeType: util.dataTypes[columns[name]] };
        });
    ...
    this.create = function(options, cb) {
        ...
        Object.values(me.definition).forEach(table => {
            ...
            if (table.behaviors) {
                if (!Array.isArray(table.behaviors)) {
                    table.behaviors = [ table.behaviors ];
                }
                
                table.mixins.add(table.behaviors.map(behavior => behavior(table)));
            }
        });
        ...
        me.load(me.definition, function(err) {
            if (err) cb(err);
                ...
                async.forEachSeries(Object.keys(me.definition), (table, cb) => {
                                ...
                                if (err) cb(err);
                                    ...
                                    me.describeTable(me.definition[table].name, function(err, metadata) {
                                        if (err) cb(err);
                                            ...
                                            var mixin = me.definition[table].mixin || me.definition[table].members || me.definition[table].properties || me.definition[table].methods;
                                            if (mixin) me.tables[table].mixin(mixin);
                                            
                                            if (me.definition[table].mixins) {
                                                me.definition[table].mixins.forEach(mixin => me.tables[table].mixin(mixin));
                                            }
