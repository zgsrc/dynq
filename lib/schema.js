...
module.exports = function(connection) {
    ...
    this.createTable = function(definition, cb) {
        ...
        try {
            ...
            options.AttributeDefinitions = Object.keys(columns).map(function(name) {
                return { AttributeName: name, AttributeType: dataTypes[columns[name]] };
            });
        }
        catch (ex) {
            cb(ex);
    ...
    this.factorThroughput = function(description, factor, cb) {
        var update = {
            ...
            ProvisionedThroughput: {
                ReadCapacityUnits: Math.round(factor * description.ProvisionedThroughput.ReadCapacityUnits),
                WriteCapacityUnits: Math.round(factor * description.ProvisionedThroughput.WriteCapacityUnits)
            }
        };
        
        if (description.GlobalSecondaryIndexes.length) {
            update.GlobalSecondaryIndexUpdates = [ ];
            description.GlobalSecondaryIndexes.forEach(function(index) {
        ...
        me.connection.updateTable(update, cb);
    };
                ...
                if (filter) {
                    if (Object.isFunction(filter) || Object.isRegExp(filter)) {
                        ...
                        Object.keys(filter).forEach((key) => { 
                            candidates[(filter[key].name || filter[key])] = key; 
                        });
